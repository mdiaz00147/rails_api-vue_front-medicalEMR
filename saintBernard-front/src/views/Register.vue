    ...
    <div class="jumbotron">
      ...
      <form  class="text-left row" @submit="onSubmit">
        ...
        <div class="form-group">
          ...
          <input v-model="form.password_confiramtion" type="password" class="form-control" id="exampleInputPassword1" placeholder="Password">
        </div>

        <button v-if="!loading" type="submit" class="btn btn-success col-md-6">Register</button>
        <button v-if="loading" type="submit" class="btn btn-success col-md-6" disabled>Loading <i class="fa fa-spinner fa-spin"></i></button>
        <router-link to="/" class="btn btn-primary col-md-6">Sign In</router-link>
      </form>
    </div>
...
export default {
  ...
  methods: {
    onSubmit (evt) {
      evt.preventDefault()
      this.loading = true
      axios.post(config.defaultURL + '/v1/client/users', this.form)
      .then(response => {
